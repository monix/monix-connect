...
@InternalApi
private[dynamodb] object AsyncClientConversions { self =>
  ...
  def from(
    ...
    httpClient: Option[SdkAsyncHttpClient]): DynamoDbAsyncClient = {
    val builder = DynamoDbAsyncClient.builder().credentialsProvider(credentialsProvider).region(region)
