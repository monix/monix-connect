...
Listing a very long list of objects might take time and might cause errors to the underlying netty connection due to a request timeout,
in order avoid that, first you might want to set a maximum total of keys fetched, or secondly you could also increase the timeouts
...
### List latest and oldest

A variant of the list objects method is also exposed in this connector.
It might be handy in some cases in which for example we have a list of files under
the same path in `S3`, but we are only interested in the latest updated one,
in such cases you could just make use of `listLatestObject` or `listLatestNObjects`.
On the other hand if the interest is on the oldest objects, we could rather use `listOldestObject` or `listOldestNObjects`.
