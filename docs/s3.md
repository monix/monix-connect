...
## Dependency
 ...
 ```scala
 libraryDependencies += "io.monix" %% "monix-s3" % "0.5.0-RC1"
 ```
...
## Async Client
 ...
 There are different ways to create the connection, which is implemented under the singleton object `monix.connect.s3.S3` and explained in more detail in the following sub-sections:
 
 ### Create from config
  
  The laziest and better way to create the _S3_ connection is to do it from configuration file.
  To do so, you'll just need to create an `application.conf` following the template [reference.conf](https://github.com/monix/monix-connect/tree/master/aws-auth/src/main/resources/reference.conf).
...
therefore it will be automatically picked up from the method call `S3.create()`, which will return a `cats.effect.Resource[Task, S3]`.
The [resource](https://typelevel.org/cats-effect/datatypes/resource.html) will the *creation* and *release* of the _S3_ connection, 
and the easiest way to use it is to define your application (methods and classes) expecting an instance of _S3_ that will be passed from within the 
_usage_ of the _Resource_. See below code snippet a simple app example:
...
```  
 ...
 val s3: Resource[Task, S3] = S3.createWith(staticCredProvider, Region.AWS_GLOBAL)   
```
...
Finally, just another variant of the builders shown previously:
 ```scala
import software.amazon.awssdk.auth.credentials.DefaultCredentialsProvider
import software.amazon.awssdk.regions.Region
import monix.connect.s3.S3
...
val s3: S3 = S3.createUnsafeWith(DefaultCredentialsProvider.create(), Region.AF_SOUTH_1)
```
...
val bucket: String = "my-bucket" 
val t: Task[CreateBucketResponse] = S3.create().use(_.createBucket(bucket))
```
...
val bucket: String = "my-bucket" 
val t: Task[DeleteBucketResponse] = S3.create().use(_.deleteBucket(bucket))
```
...
val t: Task[DeleteObjectResponse] = S3.create().use(_.deleteObject(bucket, key))
```
...
val t: Task[Boolean] = S3.create().use(_.existsBucket(bucket))
```
...
val t: Task[Boolean] = S3.create().use(_.existsObject(bucket, key))
```
...
val totalSize: Task[Long] = S3.create().use{s3 =>
...
val s3ObjectsWithLimit: Observable[S3Object] = 
  S3.createUnsafe(s3AsyncClient).listObjects(bucket, maxTotalKeys = Some(1011), prefix = Some(prefix))
```
...
val t: Task[Array[Byte]] = S3.create().use(s3 => s3.download("my-bucket", key))
```
...
val t: Task[PutObjectResponse] = S3.create().use(_.upload("my-bucket", key, content))
```
...
val s3: S3 = S3.createUnsafeWith(new AnonymousCredentialsProvider, Region.EU_WEST_1, Some(URI.create(localStackEndpoint)))
 ``` 
...
val s3: S3 = S3.createUnsafeWith(basicAWSCredentials, Region.EU_WEST_1, Some(URI.create(minioEndPoint)))
```
