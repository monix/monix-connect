...
val bucket: String = "my-bucket" 
val t: Task[DeleteBucketResponse] = S3.fromConfig.use(_.deleteBucket(bucket))
```
...
val t: Task[DeleteObjectResponse] = S3.fromConfig.use(_.deleteObject(bucket, key))
```
...
val t: Task[Boolean] = S3.fromConfig.use(_.existsBucket(bucket))
```
...
val t: Task[Boolean] = S3.fromConfig.use(_.existsObject(bucket, key))
```
...
val s3ObjectsWithLimit: Observable[S3Object] = 
  S3.createUnsafe(credentialsProvider, region).listObjects(bucket, maxTotalKeys = Some(1011), prefix = Some(prefix))
```
...
val t: Task[Array[Byte]] = S3.fromConfig.use(s3 => s3.download("my-bucket", key))
```
...
val t: Task[PutObjectResponse] = S3.fromConfig.use(_.upload("my-bucket", key, content))
```
