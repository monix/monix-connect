...
object SnakeCaseConfigReader {
  private[auth] implicit val staticCreedsConfConfigReader: ConfigReader[StaticCredentialsConf] =
    ConfigReader.forProduct3("access_key_id", "secret_access_key", "session-token")(StaticCredentialsConf)
  private[auth] implicit val awsCredentialsConfConfigReader: ConfigReader[AwsCredentialsConf] =
    ConfigReader.forProduct3("provider", "profile_name", "static")(AwsCredentialsConf)
  ...
  implicit val appConfConfigReader = ConfigReader.forProduct1("monix_aws")(AppConf)
}
