...
@Warmup(iterations = 1)
@Fork(1)
class Redis4CatsListsBenchmark extends Redis4CatsBenchFixture {
  ...
  def setup(): Unit = {
    ...
    (1 to maxKey).foreach { key =>
      ...
      val f = redis4catsConn.use(c => c.rPush(key.toString, value)).unsafeToFuture
  ...
  def listRangeReader(): Unit = {
    val f = redis4catsConn.use(c => c.lRange(keysCycle.next, 0, 1)).unsafeToFuture
    Await.ready(f, 1.seconds)
  }
}
