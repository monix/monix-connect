...
@Warmup(iterations = 1)
@Fork(1)
class RedisLaserdiscHashesBenchmark extends RedisLaserdiscBenchFixture {
  ...
  def hashFieldValueReader(): Unit = {
    val f = laserdConn
      .use(c => c.send(cmd.hget[String](Key.unsafeFrom(keysCycle.next), Key.unsafeFrom(getRandomString))))
      .unsafeToFuture
    Await.ready(f, 1.seconds)
  }
  ...
  def hashAllReader(): Unit = {
    val f = laserdConn.use(c => c.send(cmd.hgetall(Key.unsafeFrom(keysCycle.next)))).unsafeToFuture
    Await.ready(f, 1.seconds)
  }
}
