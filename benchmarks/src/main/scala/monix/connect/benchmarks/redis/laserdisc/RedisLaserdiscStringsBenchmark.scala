...
@Warmup(iterations = 1)
@Fork(1)
class RedisLaserdiscStringsBenchmark extends RedisLaserdiscBenchFixture {
  ...
  def setup(): Unit = {
    ...
    (1 to maxKey).foreach { key =>
      ...
      val f = laserdConn
        .use(c => c.send(cmd.set[String](Key.unsafeFrom(key.toString), value)))
        .unsafeToFuture
  ...
  def stringAppender(): Unit = {
    val value = getRandomString
    val f = laserdConn
      .use(c => c.send(cmd.append[String](Key.unsafeFrom(keysCycle.next), value)))
      .unsafeToFuture
    Await.ready(f, 1.seconds)
  }
