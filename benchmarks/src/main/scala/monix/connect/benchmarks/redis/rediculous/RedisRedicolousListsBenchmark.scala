...
@Warmup(iterations = 1)
@Fork(1)
class RedisRedicolousListsBenchmark extends RedisRedicolousBenchFixture {
  ...
  def setup(): Unit = {
    ...
    (1 to maxKey).foreach { key =>
      ...
      val f = redicolousConn
        .use(c => RedisCommands.rpush[RedisIO](key.toString, List(value)).run(c))
        .unsafeToFuture
  ...
  def listRangeReader(): Unit = {
    val f = redicolousConn
      .use(c => RedisCommands.lrange[RedisIO](keysCycle.next, 0, 1).run(c))
      .unsafeToFuture
    Await.ready(f, 1.seconds)
  }
}
