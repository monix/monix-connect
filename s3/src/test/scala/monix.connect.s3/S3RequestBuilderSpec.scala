...
class S3RequestBuilderSpec
  ...
  s"correctly build `GetObjectRequest`s" in {
    //given
    val (bucket: String, key: String, nBytes: Option[String], downloadSettings: DownloadSettings) =
      genGetObjectParams.sample.get
  ...
  s"correctly build `PutObjectRequest`" in {
    val (bucket: String, key: String, contentLenght: Option[Long], uploadSettings: UploadSettings) =
      genPutObjectParams.sample.get
    //when
