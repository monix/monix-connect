...
class InboundMessageSpec extends AnyFlatSpecLike with Matchers with SqsFixture {
  ...
  "A single fifo message" can "be converted to a java `MessageRequest`" in {
    ...
    messageRequest.messageAttributes().asScala.toMap shouldBe messageAttributes.map{ case (k, v) => (k, v.toAttrValue)}
    messageRequest.hasMessageSystemAttributes shouldBe awsTraceHeader.isDefined
    messageRequest.messageSystemAttributes().asScala.toMap shouldBe awsTraceHeader.map(header => Map(MessageSystemAttributeNameForSends.AWS_TRACE_HEADER -> header.toSystemAttrValue)).getOrElse(Map.empty)
  }
  ...
  "MessageSystemAttributeNameForSends" should "only be formed by awsTraceHeader" in {
    ...
    MessageSystemAttributeNameForSends.values() shouldBe Array(MessageSystemAttributeNameForSends.AWS_TRACE_HEADER, MessageSystemAttributeNameForSends.UNKNOWN_TO_SDK_VERSION)
  }
  ...
  "A StringMessageAttribute" can "be converted to java aws" in {
    ...
    stringMessageAttr.toSystemAttrValue shouldBe MessageSystemAttributeValue.builder().stringValue(attributeValue).dataType("String").build()
    stringMessageAttr.toAttrValue shouldBe MessageAttributeValue.builder().stringValue(attributeValue).dataType("String").build()
  }
  ...
  "A BinaryMessageAttribute" can "be converted to java aws" in {
    ...
    binaryMessageAttr.toSystemAttrValue shouldBe MessageSystemAttributeValue.builder().binaryValue(SdkBytes.fromByteArray(attributeValue)).dataType("Binary").build()
    binaryMessageAttr.toAttrValue shouldBe MessageAttributeValue.builder().binaryValue(SdkBytes.fromByteArray(attributeValue)).dataType("Binary").build()
  }
}
