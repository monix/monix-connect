...
class SinkSuite extends AnyFlatSpecLike with Matchers with ScalaFutures with SqsFixture with BeforeAndAfterAll {
  ...
  "A stream of `N` messages" must "be sent with the producer sink subscriber" in {
    val groupId = "groupId"
  ...
  "A group of more than 10 messages" must "be split and sent in batches of at most 10 entries" in {
    val groupId = "groupId"
    val queueName = genFifoQueueName.sample.get
